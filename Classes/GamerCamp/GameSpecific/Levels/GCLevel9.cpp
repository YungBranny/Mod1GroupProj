...
#include "GamerCamp/GameSpecific/Levels/GCLevel10.h"
...
CGCLevel9::CGCLevel9 ()
	: IGCGameLayer (GetGCTypeIDOf (CGCLevel9))
	, m_pcGCTimer (nullptr)
...
void CGCLevel9::keyCollected () // Mia: This function adds one more Key onto how many the Player obtains
{
...
void CGCLevel9::playBackgroundMusic () // Mia: Function that is called when we want the Background Music to play
{
	m_pcGCBackgroundAudio = CocosDenshion::SimpleAudioEngine::getInstance ();
	m_pcGCBackgroundAudio->playBackgroundMusic ("Sounds/BackgroundMusic/CrystalCoralReefSvRV3.wav", true); // Mia: Play Audio by locating File, set to 'True' to loop
}

void CGCLevel9::playKeyAudio () // Mia: Function that is called when we want the Collected Key Sound Effect to play
{
...
void CGCLevel9::playDoorOpeningAudio () // Mia: Function that is called when we want the Door Opened Sound Effect to play
{
	m_pcGCSoundEffectsAudio = CocosDenshion::SimpleAudioEngine::getInstance ();
...
void CGCLevel9::VOnCreate ()
{
	...
	MenuItemImage* pResetItem // Mia: This button resets the Level
		= MenuItemImage::create ("Loose/CloseNormal.png",
			...
			CC_CALLBACK_1 (CGCLevel9::Callback_OnResetButton, this));
	...
	MenuItemImage* pSkipItem // Mia: This button skips a Level onto the next
		= MenuItemImage::create ("Loose/CloseNormal.png",
			"Loose/CloseSelected.png",
			CC_CALLBACK_1 (CGCLevel9::Callback_OnSkipButton, this));
	...
	MenuItemImage* pQuitItem // Mia: This button quits the Game back onto the Main Menu
		= MenuItemImage::create ("Loose/CloseNormal.png",
			"Loose/CloseSelected.png",
			CC_CALLBACK_1 (CGCLevel9::Callback_OnQuitButton, this));
	...
	// Loads Level
	m_cLevelLoader.LoadLevelFile (FileUtils::getInstance ()->fullPathForFilename (std::string ("OgmoEditor/Level9.oel")).c_str ());
...
void CGCLevel9::PlayerDeathSceneSwap ()
{
...
void CGCLevel9::HighScore ()
{


}

void CGCLevel9::VOnUpdate (f32 fTimeStep)
{
	...
	{
		ReplaceScene (TransitionMoveInR::create (0.1f, TGCGameLayerSceneCreator< CGCLevel10 >::CreateScene ()));
	}
...
void CGCLevel9::Callback_OnQuitButton (Ref* pSender)
{
	RequestQuit ();
}


void CGCLevel9::Callback_OnSkipButton (Ref* pSender)
{
	RequestSkip ();
}

void CGCLevel9::Callback_OnResetButton (Ref* pSender)
{
	RequestReset ();
}
...
void CGCLevel9::BeginContact (b2Contact* pB2Contact)
{
...
void CGCLevel9::EndContact (b2Contact* pB2Contact)
{
	{
		const b2Fixture* pFixtureA = pB2Contact->GetFixtureA ();
		const b2Fixture* pFixtureB = pB2Contact->GetFixtureB ();
...
void CGCLevel9::PreSolve (b2Contact* pB2Contact, const b2Manifold* pOldManifold)
{
	const b2Fixture* pFixtureA = pB2Contact->GetFixtureA ();
	const b2Fixture* pFixtureB = pB2Contact->GetFixtureB ();
...
void CGCLevel9::PostSolve (b2Contact* pB2Contact, const b2ContactImpulse* pImpulse)
{
}
...
void CGCLevel9::HandleCollisions ()
{
