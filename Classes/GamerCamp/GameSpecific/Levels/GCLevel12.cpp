...
#include "GamerCamp/GameSpecific/Levels/GCLevel13.h"
...
CGCLevel12::CGCLevel12 ()
	: IGCGameLayer (GetGCTypeIDOf (CGCLevel12))
	, m_pcGCTimer (nullptr)
...
void CGCLevel12::keyCollected () // Mia: This function adds one more Key onto how many the Player obtains
{
...
void CGCLevel12::playBackgroundMusic () // Mia: Function that is called when we want the Background Music to play
{
	m_pcGCBackgroundAudio = CocosDenshion::SimpleAudioEngine::getInstance ();
	m_pcGCBackgroundAudio->playBackgroundMusic ("Sounds/BackgroundMusic/Shipwreck_v3 - Ambient.wav", true); // Mia: Play Audio by locating File, set to 'True' to loop
}

void CGCLevel12::playKeyAudio () // Mia: Function that is called when we want the Collected Key Sound Effect to play
{
...
void CGCLevel12::playDoorOpeningAudio () // Mia: Function that is called when we want the Door Opened Sound Effect to play
{
	m_pcGCSoundEffectsAudio = CocosDenshion::SimpleAudioEngine::getInstance ();
...
void CGCLevel12::VOnCreate ()
{
	...
	MenuItemImage* pResetItem // Mia: This button resets the Level
		= MenuItemImage::create ("Loose/CloseNormal.png",
			...
			CC_CALLBACK_1 (CGCLevel12::Callback_OnResetButton, this));
	...
	MenuItemImage* pSkipItem // Mia: This button skips a Level onto the next
		= MenuItemImage::create ("Loose/CloseNormal.png",
			"Loose/CloseSelected.png",
			CC_CALLBACK_1 (CGCLevel12::Callback_OnSkipButton, this));
	...
	MenuItemImage* pQuitItem // Mia: This button quits the Game back onto the Main Menu
		= MenuItemImage::create ("Loose/CloseNormal.png",
			"Loose/CloseSelected.png",
			CC_CALLBACK_1 (CGCLevel12::Callback_OnQuitButton, this));
	...
	// Loads Level
	m_cLevelLoader.LoadLevelFile (FileUtils::getInstance ()->fullPathForFilename (std::string ("OgmoEditor/Level12.oel")).c_str ());
...
void CGCLevel12::PlayerDeathSceneSwap ()
{
...
void CGCLevel12::HighScore ()
{


}

void CGCLevel12::VOnUpdate (f32 fTimeStep)
{
	...
	{
		ReplaceScene (TransitionMoveInR::create (0.1f, TGCGameLayerSceneCreator< CGCLevel13 >::CreateScene ()));
	}
...
void CGCLevel12::Callback_OnQuitButton (Ref* pSender)
{
	RequestQuit ();
}


void CGCLevel12::Callback_OnSkipButton (Ref* pSender)
{
	RequestSkip ();
}

void CGCLevel12::Callback_OnResetButton (Ref* pSender)
{
	RequestReset ();
}
...
void CGCLevel12::BeginContact (b2Contact* pB2Contact)
{
...
void CGCLevel12::EndContact (b2Contact* pB2Contact)
{
	{
		const b2Fixture* pFixtureA = pB2Contact->GetFixtureA ();
		const b2Fixture* pFixtureB = pB2Contact->GetFixtureB ();
...
void CGCLevel12::PreSolve (b2Contact* pB2Contact, const b2Manifold* pOldManifold)
{
	const b2Fixture* pFixtureA = pB2Contact->GetFixtureA ();
	const b2Fixture* pFixtureB = pB2Contact->GetFixtureB ();
...
void CGCLevel12::PostSolve (b2Contact* pB2Contact, const b2ContactImpulse* pImpulse)
{
}
...
void CGCLevel12::HandleCollisions ()
{
